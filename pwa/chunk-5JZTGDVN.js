...
function combineReducers(reducers, initialState3 = {}) {
  ...
  return function combination(state, action) {
    state = state === void 0 ? initialState3 : state;
...
var ReducerManager = class _ReducerManager extends BehaviorSubject {
  ...
  constructor(dispatcher, initialState3, reducers, reducerFactory) {
    super(reducerFactory(reducers, initialState3));
    this.dispatcher = dispatcher;
    this.initialState = initialState3;
  ...
  addFeatures(features) {
    const reducers = features.reduce((reducerDict, {
      ...
      const reducer = typeof reducers2 === "function" ? createFeatureReducerFactory(metaReducers)(reducers2, initialState3) : createReducerFactory(reducerFactory, metaReducers)(reducers2, initialState3);
...
var State = class _State extends BehaviorSubject {
  ...
  constructor(actions$, reducer$, scannedActions, initialState3) {
    super(initialState3);
...
function _initialStateFactory(initialState3) {
...
function createReducer(initialState3, ...ons) {
  ...
  return function(state = initialState3, action) {
...
var STORE_KEY = "[GLOBAL STORE]";
var PLAYLISTS_STORE_KEY = "[PLAYLISTS STORE]";
var loadPlaylists = createAction(`${PLAYLISTS_STORE_KEY} Load playlists from db`);
var loadPlaylistsSuccess = createAction(`${PLAYLISTS_STORE_KEY} Successfully loaded playlists from db`, props());
var addPlaylist = createAction(`${PLAYLISTS_STORE_KEY} Add new playlist`, props());
var addManyPlaylists = createAction(`${PLAYLISTS_STORE_KEY} Add many playlists`, props());
var removePlaylist = createAction(`${PLAYLISTS_STORE_KEY} Remove playlist by id`, props());
var updatePlaylistMeta = createAction(`${PLAYLISTS_STORE_KEY} update playlist meta`, props());
var updatePlaylist = createAction(`${PLAYLISTS_STORE_KEY} update playlist`, props());
var updateManyPlaylists = createAction(`${PLAYLISTS_STORE_KEY} Update many playlists (auto-update mechanism)`, props());
var parsePlaylist = createAction(`${PLAYLISTS_STORE_KEY} parse playlist`, props());
var setActivePlaylist = createAction(`${PLAYLISTS_STORE_KEY} set active playlist`, props());
var updateFavorites = createAction(`${STORE_KEY} Add/remove provided channel to the favorites`, props());
var setFavorites = createAction(`${STORE_KEY} Set favorites`, props());
var setActiveChannel = createAction(`${STORE_KEY} Set active channel`, props());
var resetActiveChannel = createAction(`${STORE_KEY} Reset active channel`);
var setActiveChannelSuccess = createAction(`${STORE_KEY} Set active channel success`, props());
var setActiveEpgProgram = createAction(`${STORE_KEY} Sets the given timestamp for the epg program`, props());
var setCurrentEpgProgram = createAction(`${STORE_KEY} Updates the active epg program for the active channel`, props());
var resetActiveEpgProgram = createAction(`${STORE_KEY} Reset active epg program`);
var setEpgAvailableFlag = createAction(`${STORE_KEY} Reset active epg program`, props());
var setChannels = createAction(`${STORE_KEY} Set channels`, props());
var updatePlaylistPositions = createAction(`${STORE_KEY} Update playlist positions`, props());
var removeAllPlaylists = createAction(`${STORE_KEY} Remove all playlists`);
var setAdjacentChannelAsActive = createAction(`${STORE_KEY} Set adjacent channel as active`, props());
var setSelectedFilters = createAction(`${STORE_KEY} Set selected filters`, props());
...
var Database = class _Database {
  ...
  static load(path) {
    return __async(this, null, function* () {
      const _path = yield invoke("plugin:sql|load", {
        db: path
      });
  ...
  execute(query, bindValues) {
    return __async(this, null, function* () {
      const [rowsAffected, lastInsertId] = yield invoke("plugin:sql|execute", {
        db: this.path,
        query,
        values: bindValues ?? []
      });
  ...
  select(query, bindValues) {
    return __async(this, null, function* () {
      const result = yield invoke("plugin:sql|select", {
        db: this.path,
        query,
        values: bindValues ?? []
      });
  ...
  close(db) {
    return __async(this, null, function* () {
      const success = yield invoke("plugin:sql|close", {
        db
      });
      return success;
    });
...
var DatabaseService = class _DatabaseService {
  ...
  getConnection() {
    return __async(this, null, function* () {
      if (!_DatabaseService.db) {
        _DatabaseService.db = yield Database.load("sqlite:database.db");
      }
  ...
  deletePlaylist(playlistId) {
    return __async(this, null, function* () {
      try {
        ...
        yield db.execute("BEGIN TRANSACTION");
        try {
          yield db.execute(`
          ...
          yield db.execute(`
                ...
                `, [playlistId]);
          yield db.execute("DELETE FROM categories WHERE playlist_id = ?", [playlistId]);
          yield db.execute("DELETE FROM playlists WHERE id = ?", [
            playlistId
          ]);
  ...
  updateXtreamPlaylist(playlist) {
    return __async(this, null, function* () {
      try {
        const db = yield this.getConnection();
        yield db.execute("UPDATE playlists SET name = ? WHERE id = ?", [
          playlist.name,
          playlist.id
        ]);
        return true;
  ...
  updateXtreamPlaylistDetails(playlist) {
    return __async(this, null, function* () {
      try {
        const db = yield this.getConnection();
        const updateFields = ["name = ?"];
        const params = [playlist.title];
  ...
  hasXtreamCategories(playlistId, type) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      const result = yield db.select("SELECT * FROM categories WHERE playlist_id = ? AND type = ?", [playlistId, type]);
      return result.length > 0;
    });
  }
  getXtreamCategories(playlistId, type) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      return yield db.select("SELECT * FROM categories WHERE playlist_id = ? AND type = ? ORDER BY name COLLATE NOCASE", [playlistId, type]);
    });
  }
  saveXtreamCategories(playlistId, categories, type) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      for (const category of categories) {
        yield db.execute("INSERT INTO categories (playlist_id, name, type, xtream_id) VALUES (?, ?, ?, ?)", [playlistId, category.category_name, type, category.category_id]);
      }
    });
  }
  hasXtreamContent(playlistId, type) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      const result = yield db.select(`SELECT c.* FROM content c 
             JOIN categories cat ON c.category_id = cat.id 
             WHERE cat.playlist_id = ? AND c.type = ?
             ORDER BY c.added`, [playlistId, type]);
      return result.length > 0;
    });
  }
  getXtreamContent(playlistId, type) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      return yield db.select(`SELECT 
                c.id, c.category_id, c.title, c.rating, 
                c.added, c.poster_url, c.xtream_id, c.type
            FROM content c 
            INNER JOIN categories cat ON c.category_id = cat.id 
            WHERE cat.playlist_id = ? AND c.type = ?
            ORDER BY c.added DESC`, [playlistId, type]);
    });
  }
  saveXtreamContent(playlistId, streams, type, onProgress) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      const dbType = type === "series" ? "series" : type === "movie" ? "movies" : "live";
      const categories = yield db.select("SELECT id, xtream_id FROM categories WHERE playlist_id = ? AND type = ?", [playlistId, dbType]);
      const categoryMap = new Map(categories.map((c) => [parseInt(c.xtream_id.toString()), c.id]));
      const bulkInsertData = this.prepareBulkInsertData(streams, type, categoryMap);
      return yield this.executeBulkInsert(db, bulkInsertData, onProgress);
    });
  }
  searchXtreamContent(playlistId, searchTerm, types) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      const placeholders = types.map(() => "?").join(",");
      return yield db.select(`SELECT c.* FROM content c 
             JOIN categories cat ON c.category_id = cat.id 
             WHERE (c.title LIKE ?)
             AND cat.playlist_id = ?
             AND c.type IN (${placeholders})
             LIMIT 50`, [`%${searchTerm}%`, playlistId, ...types]);
    });
  }
  globalSearchContent(searchTerm, types) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      const placeholders = types.map(() => "?").join(",");
      return yield db.select(`
  ...
  clearGlobalRecentlyViewed() {
    return __async(this, null, function* () {
      try {
        const db = yield this.getConnection();
        yield db.execute("DELETE FROM recently_viewed");
      } catch (error) {
  ...
  getPlaylistById(playlistId) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      const results = yield db.select("SELECT * FROM playlists WHERE id = ?", [playlistId]);
  ...
  createPlaylist(playlist) {
    return __async(this, null, function* () {
      const db = yield this.getConnection();
      yield db.execute("INSERT INTO playlists (id, name, serverUrl, username, password, type) VALUES (?, ?, ?, ?, ?, ?)", [
        playlist._id,
        playlist.title,
        playlist.serverUrl,
        playlist.username,
  ...
  prepareBulkInsertData(streams, type, categoryMap) {
    return streams.map((stream) => {
      const streamCategoryId = type === "series" ? parseInt(stream.category_id || "0") : parseInt(stream.category_id);
      const categoryId = categoryMap.get(streamCategoryId);
      if (!categoryId)
        return null;
      const title = type === "series" ? stream.title || stream.name || `Unknown Series ${stream.series_id}` : stream.name || stream.title || `Unknown Stream ${stream.stream_id}`;
      return [
        categoryId,
        title,
        stream.rating || stream.rating_imdb || "",
        type === "series" ? stream.last_modified || "" : stream.added || "",
        stream.stream_icon || stream.poster || stream.cover || "",
        type === "series" ? parseInt(stream.series_id || "0") : parseInt(stream.stream_id || "0"),
        type
      ];
    }).filter((data) => data !== null);
  }
  executeBulkInsert(db, data, onProgress) {
    return __async(this, null, function* () {
      ...
      for (let i = 0; i < data.length; i += CHUNK_SIZE) {
        ...
        const query = `
            ...
            `;
        try {
          yield db.execute(query, chunk.flat());
  ...
  static {
    this.\u0275fac = function DatabaseService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _DatabaseService)();
    };
  }
  static {
...
// shared/constants.ts
var GLOBAL_FAVORITES_PLAYLIST_ID = "GLOBAL_FAVORITES";
...
function createSelectorsFactory() {
  function getSelectors(selectState) {
    const selectIds2 = (state) => state.ids;
    const selectEntities = (state) => state.entities;
    const selectAll = createSelector(selectIds2, selectEntities, (ids, entities) => ids.map((id) => entities[id]));
    const selectTotal2 = createSelector(selectIds2, (ids) => ids.length);
    if (!selectState) {
      return {
    ...
    return {
      selectIds: createSelector(selectState, selectIds2),
      selectEntities: createSelector(selectState, selectEntities),
      selectAll: createSelector(selectState, selectAll),
      selectTotal: createSelector(selectState, selectTotal2)
    };
  }
  return {
    getSelectors
...
// src/app/state/playlists.state.ts
var playlistsAdapter = createEntityAdapter({
  selectId: (item) => item._id
});
var initialPlaylistMetaState = playlistsAdapter.getInitialState({
  selectedId: "",
  allPlaylistsLoaded: false,
  selectedFilters: ["m3u", "xtream", "stalker"]
});
var { selectIds, selectEntities: getPlaylistMetaEntities, selectAll: getAllPlaylistsMeta, selectTotal } = playlistsAdapter.getSelectors();
...
// src/app/state/reducers.ts
var playlistReducer = createReducer(initialState, on(setActiveEpgProgram, (state, action) => {
  const { program } = action;
  const from2 = (0, import_moment.default)(program.start, "YYYYMMDDHHmm ZZ").unix();
  const now = (0, import_moment.default)(Date.now()).unix();
  const epgParams = `?utc=${from2}&lutc=${now}`;
  return __spreadProps(__spreadValues({}, state), {
    active: __spreadProps(__spreadValues({}, state.active), { epgParams })
  });
}), on(resetActiveEpgProgram, (state) => __spreadProps(__spreadValues({}, state), {
  active: __spreadProps(__spreadValues({}, state.active), { epgParams: "" })
})), on(setActiveChannelSuccess, (state, action) => {
  const { channel } = action;
  return __spreadProps(__spreadValues({}, state), {
    active: __spreadProps(__spreadValues({}, channel), { epgParams: "" })
  });
}), on(resetActiveChannel, (state) => {
  return __spreadProps(__spreadValues({}, state), {
    active: void 0
  });
}), on(setCurrentEpgProgram, (state, action) => __spreadProps(__spreadValues({}, state), {
  currentEpgProgram: action.program
})), on(setEpgAvailableFlag, (state, action) => __spreadProps(__spreadValues({}, state), {
  epgAvailable: action.value
})), on(updateFavorites, (state, action) => {
  let favorites;
  const { channel } = action;
  const playlistFavorites = state.playlists.entities[state.playlists.selectedId].favorites;
  ...
  return __spreadProps(__spreadValues({}, state), {
    playlists: __spreadProps(__spreadValues({}, state.playlists), {
      entities: __spreadProps(__spreadValues({}, state.playlists.entities), {
        [state.playlists.selectedId]: __spreadProps(__spreadValues({}, state.playlists.entities[state.playlists.selectedId]), {
          favorites
        })
      })
    })
  });
}), on(setFavorites, (state, action) => {
  ...
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.addMany(action.playlists, __spreadProps(__spreadValues({}, state.playlists), {
      allPlaylistsLoaded: true
    }))
  });
}), on(removePlaylist, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.removeOne(action.playlistId, state.playlists)
  });
}), on(updatePlaylist, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.updateOne({
      id: action.playlistId,
      changes: __spreadProps(__spreadValues({}, action.playlist), {
  ...
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.addOne(action.playlist, state.playlists)
  });
}), on(addManyPlaylists, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.addMany(action.playlists, state.playlists)
  });
}), on(setActivePlaylist, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: __spreadProps(__spreadValues({}, state.playlists), {
      selectedId: action.playlistId
    })
  });
}), on(updatePlaylistPositions, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.updateMany(action.positionUpdates, state.playlists)
  });
}), on(updatePlaylistMeta, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.updateOne({
      id: action.playlist._id,
      changes: __spreadValues(__spreadValues(__spreadValues(__spreadValues(__spreadValues({
        title: action.playlist.title,
        autoRefresh: action.playlist.autoRefresh || false,
        userAgent: action.playlist.userAgent
      }, action.playlist.serverUrl !== null ? { serverUrl: action.playlist.serverUrl } : {}), action.playlist.username !== null ? { username: action.playlist.username } : {}), action.playlist.password !== null ? { password: action.playlist.password } : {}), action.playlist.macAddress !== null ? { macAddress: action.playlist.macAddress } : {}), action.playlist.portalUrl !== null ? { portalUrl: action.playlist.portalUrl } : {})
    }, state.playlists)
  });
}), on(setChannels, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    channels: action.channels
  });
}), on(updateManyPlaylists, (state, action) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.updateMany(action.playlists.map((updatedPlaylist) => ({
      id: updatedPlaylist._id,
      changes: __spreadProps(__spreadValues({}, updatedPlaylist), {
        updateDate: Date.now()
      })
    })), state.playlists)
  });
}), on(removeAllPlaylists, (state) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: playlistsAdapter.removeAll(state.playlists)
  });
}), on(setSelectedFilters, (state, { selectedFilters }) => {
  return __spreadProps(__spreadValues({}, state), {
    playlists: __spreadProps(__spreadValues({}, state.playlists), {
      selectedFilters
    })
...
var selectIsEpgAvailable = (state) => state.epgAvailable;
var selectActive = (state) => state.active;
var selectCurrentEpgProgram = (state) => state.currentEpgProgram;
var selectChannels = (state) => state.channels;
var selectPlaylists = (state) => state.playlists;
var selectPlaylistId = (state) => state.playlists?.selectedId;
var selectPlaylistsLoadingFlag = (state) => state.playlists?.allPlaylistsLoaded;
...
function isSameUrl(first2, second) {
  return stripTrailingSlash(first2) === stripTrailingSlash(second);
}
...
function getRouterSelectors(selectState = createRouterSelector()) {
  ...
  const selectCurrentRoute2 = createSelector(selectRootRoute, (rootRoute2) => {
    if (!rootRoute2) {
      return void 0;
    }
    let route = rootRoute2;
    while (route.firstChild) {
      route = route.firstChild;
    }
    return route;
  });
  const selectFragment2 = createSelector(selectRootRoute, (route) => route && route.fragment);
  const selectQueryParams2 = createSelector(selectRootRoute, (route) => route && route.queryParams);
  const selectQueryParam2 = (param) => createSelector(selectQueryParams2, (params) => params && params[param]);
  const selectRouteParams2 = createSelector(selectCurrentRoute2, (route) => route && route.params);
  const selectRouteParam2 = (param) => createSelector(selectRouteParams2, (params) => params && params[param]);
  const selectRouteData2 = createSelector(selectCurrentRoute2, (route) => route && route.data);
  const selectRouteDataParam2 = (param) => createSelector(selectRouteData2, (data) => data && data[param]);
  const selectUrl2 = createSelector(selectRouterState, (routerState) => routerState && routerState.url);
  const selectTitle2 = createSelector(selectCurrentRoute2, (route) => {
  ...
  return {
    selectCurrentRoute: selectCurrentRoute2,
    selectFragment: selectFragment2,
    selectQueryParams: selectQueryParams2,
    selectQueryParam: selectQueryParam2,
    selectRouteParams: selectRouteParams2,
    selectRouteParam: selectRouteParam2,
    selectRouteData: selectRouteData2,
    selectRouteDataParam: selectRouteDataParam2,
    selectUrl: selectUrl2,
    selectTitle: selectTitle2
  };
}

// src/app/state/router.selectors.ts
var { selectCurrentRoute, selectFragment, selectQueryParams, selectQueryParam, selectRouteParams, selectRouteParam, selectRouteData, selectRouteDataParam, selectUrl, selectTitle } = getRouterSelectors();

// src/app/state/selectors.ts
var selectPlaylistState = createFeatureSelector("playlistState");
var selectIsEpgAvailable2 = createSelector(selectPlaylistState, selectIsEpgAvailable);
var selectActive2 = createSelector(selectPlaylistState, selectActive);
var selectCurrentEpgProgram2 = createSelector(selectPlaylistState, selectCurrentEpgProgram);
var selectChannels2 = createSelector(selectPlaylistState, selectChannels);
var selectPlaylistsLoadingFlag2 = createSelector(selectPlaylistState, selectPlaylistsLoadingFlag);
var selectPlaylistsMetaState = createSelector(selectPlaylistState, selectPlaylists);
var selectAllPlaylistsMeta = createSelector(selectPlaylistsMetaState, getAllPlaylistsMeta);
var selectActiveTypeFilters = createSelector(selectPlaylistsMetaState, getPlaylistMetaEntities, (state) => state.selectedFilters);
var selectActivePlaylistId = createSelector(selectPlaylistsMetaState, getPlaylistMetaEntities, (data) => data.selectedId);
var selectPlaylistTitle = createSelector(selectPlaylistsMetaState, getPlaylistMetaEntities, selectPlaylistId, (data) => {
  if (data.selectedId === GLOBAL_FAVORITES_PLAYLIST_ID) {
    return "Global favorites";
  } else if (data.entities && data.selectedId && data.entities[data.selectedId]) {
    return data.entities[data.selectedId].title || data.entities[data.selectedId].filename;
  } else
    return "Untitled playlist";
});
var selectPlaylistEntities = createSelector(selectPlaylistsMetaState, getPlaylistMetaEntities);
var selectCurrentPlaylist = createSelector(selectPlaylistEntities, selectRouteParam("id"), (entities, id) => {
  if (entities) {
    return entities[id];
  }
  return null;
});
var selectActivePlaylist = createSelector(selectPlaylistsMetaState, (state) => {
  if (state.entities && state.selectedId !== "") {
    return state.entities[state.selectedId];
  }
  return null;
});
var selectFavorites = createSelector(selectPlaylistsMetaState, getPlaylistMetaEntities, selectPlaylistId, (data) => {
  if (data.entities && data.selectedId && data.entities[data.selectedId]) {
    return data.entities[data.selectedId].favorites || [];
  } else
    return [];
});

// src/app/xtream-tauri/services/favorites.service.ts
var FavoritesService = class _FavoritesService {
  ...
  addToFavorites(item) {
    return __async(this, null, function* () {
      const db = yield this.dbService.getConnection();
      yield db.execute(`INSERT INTO favorites (content_id, playlist_id) 
             VALUES (?, ?)`, [item.content_id, item.playlist_id]);
    });
  }
  removeFromFavorites(contentId, playlistId) {
    return __async(this, null, function* () {
      const db = yield this.dbService.getConnection();
      yield db.execute(`DELETE FROM favorites 
             WHERE content_id = ? AND playlist_id = ?`, [contentId, playlistId]);
    });
  }
  isFavorite(contentId, playlistId) {
    return __async(this, null, function* () {
      const db = yield this.dbService.getConnection();
      const result = yield db.select(`SELECT COUNT(*) as count 
             FROM favorites f
             JOIN content c ON f.content_id = c.id
             WHERE c.xtream_id = ? AND f.playlist_id = ?`, [contentId, playlistId]);
      return result[0].count > 0;
    });
  }
  getFavorites(playlistId) {
    return from(this.dbService.getConnection()).pipe(mergeMap((db) => __async(this, null, function* () {
      return yield db.select(`SELECT 
                        c.*,
                        f.playlist_id,
                        f.added_at
                     ...
                     JOIN content c ON f.content_id = c.id
                     WHERE f.playlist_id = ?
                     ORDER BY f.added_at DESC`, [playlistId]);
    })), map((results) => results));
  }
  static {
    this.\u0275fac = function FavoritesService_Factory(__ngFactoryType__) {
      return new (__ngFactoryType__ || _FavoritesService)();
    };
  }
  static {
    this.\u0275prov = /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _FavoritesService, factory: _FavoritesService.\u0275fac, providedIn: "root" });
  }
};
...
// src/app/xtream-tauri/with-favorites.feature.ts
var withFavorites = function() {
  return signalStoreFeature(withState({
    isFavorite: false
  }), withMethods((store, dbService = inject(DatabaseService), favoritesService = inject(FavoritesService)) => ({
    toggleFavorite(xtreamId, playlistId) {
      return __async(this, null, function* () {
        let result = false;
        if (!xtreamId || !playlistId)
          return;
        const db = yield dbService.getConnection();
        const content = yield db.select("SELECT content.id FROM content INNER JOIN categories ON content.category_id = categories.id WHERE content.xtream_id = ? AND categories.playlist_id = ?", [xtreamId, playlistId]);
        if (!content || content.length === 0) {
          console.error("Content not found in database");
          return;
        }
        const contentId = content[0].id;
        const isFavorite = yield favoritesService.isFavorite(xtreamId, playlistId);
        if (isFavorite) {
          yield favoritesService.removeFromFavorites(contentId, playlistId);
          result = false;
        } else {
          yield favoritesService.addToFavorites({
    ...
    checkFavoriteStatus(xtreamId, playlistId) {
      return __async(this, null, function* () {
        if (!xtreamId || !playlistId) {
...
// src/app/xtream-tauri/with-recent-items.ts
var withRecentItems = function() {
  return signalStoreFeature(withState({
    recentItems: []
  }), withMethods((store, dbService = inject(DatabaseService)) => ({
    loadRecentItems: rxMethod(pipe(switchMap((playlist) => __async(this, null, function* () {
      if (!playlist)
        return [];
      console.log("Loading recent items for playlist", playlist.id);
      const db = yield dbService.getConnection();
      return db.select(`SELECT 
                            ...
                            ORDER BY rv.viewed_at DESC
                            LIMIT 50`, [playlist.id]);
    })), tap((items) => patchState(store, { recentItems: items }))))
  })), withMethods((store, dbService = inject(DatabaseService)) => ({
    addRecentItem: rxMethod(pipe(switchMap((_0) => __async(this, [_0], function* ({ contentId, playlist }) {
      if (!playlist().id) {
        console.error("No active playlist found");
        return;
      }
      console.log("Adding to recently viewed:", playlist().id);
      const db = yield dbService.getConnection();
      const content = yield db.select("SELECT content.id FROM content INNER JOIN categories ON content.category_id = categories.id WHERE content.xtream_id = ? AND categories.playlist_id = ?", [contentId, playlist().id]);
      if (content && content.length > 0) {
        const existing = yield db.select("SELECT recently_viewed.id FROM recently_viewed INNER JOIN content ON recently_viewed.content_id = content.id INNER JOIN categories ON content.category_id = categories.id WHERE content.id = ? AND categories.playlist_id = ?", [content[0].id, playlist().id]);
        if (existing && existing.length > 0) {
          yield db.execute("UPDATE recently_viewed SET viewed_at = CURRENT_TIMESTAMP WHERE id = ?", [existing[0].id]);
        } else {
          yield db.execute(`INSERT INTO recently_viewed (content_id, playlist_id) 
                            VALUES (?, ?)`, [content[0].id, playlist().id]);
        }
        return store.loadRecentItems({ id: playlist().id });
      }
    })))),
    clearRecentItems: rxMethod(pipe(switchMap((playlist) => __async(this, null, function* () {
      console.log("Clearing recent items for playlist", playlist.id);
      const db = yield dbService.getConnection();
      yield db.execute(`DELETE FROM recently_viewed WHERE playlist_id = ?`, [playlist.id]);
      return store.loadRecentItems({ id: playlist.id });
    })))),
    removeRecentItem: rxMethod(pipe(switchMap((_0) => __async(this, [_0], function* ({ itemId, playlistId }) {
      const db = yield dbService.getConnection();
      yield db.execute(`DELETE FROM recently_viewed WHERE id = ? AND playlist_id = ?`, [itemId, playlistId]);
      return store.loadRecentItems({ id: playlistId });
    })))),
    loadGlobalRecentItems() {
      return __async(this, null, function* () {
        try {
          const items = yield dbService.getGlobalRecentlyViewed();
          patchState(store, {
            recentItems: items || []
          });
        } catch (error) {
    ...
    clearGlobalRecentlyViewed() {
      return __async(this, null, function* () {
        try {
          yield dbService.clearGlobalRecentlyViewed();
          patchState(store, { recentItems: [] });
        } catch (error) {
          console.error("Error clearing global recently viewed:", error);
        }
...
// src/app/xtream-tauri/xtream.store.ts
var initialState2 = {
...
var XtreamStore = signalStore({ providedIn: "root" }, withState(initialState2), withRecentItems(), withFavorites(), withComputed((store) => ({
  getCategoriesBySelectedType: computed(() => {
    const type = store.selectedContentType();
    return type === "live" ? store.liveCategories() : type === "vod" ? store.vodCategories() : store.serialCategories();
  }),
  ...
  getSelectedItemById: computed(() => {
    ...
    const content = categoryType === "live" ? store.liveStreams() : categoryType === "vod" ? store.vodStreams() : store.serialStreams();
    if (!store.selectedItem())
      return null;
    return content.find((item) => item.stream_id === store.selectedItem().stream_id || item.id === store.selectedItem().id);
  }),
  getPaginatedContent: computed(() => {
    ...
    const categoryType = store.selectedContentType();
    const content = categoryType === "live" ? store.liveStreams() : categoryType === "vod" ? store.vodStreams() : store.serialStreams();
    return content.filter((item) => item.category_id === categoryId).slice(startIndex, endIndex);
  }),
  getTotalPages: computed(() => {
    ...
    if (!categoryId)
      return 0;
    const categoryType = store.selectedContentType();
    const totalItems = categoryType === "live" ? store.liveStreams().filter((i) => Number(i.id) === categoryId).length : categoryType === "vod" ? store.vodStreams().filter((i) => Number(i.category_id) === categoryId).length : store.serialStreams().filter((i) => i.category_id === categoryId).length;
...
})), withMethods((store, route = inject(ActivatedRoute), dataService = inject(DataService), dbService = inject(DatabaseService), oldStore = inject(Store)) => {
  const fetchCategories = (action, stateKey, type) => {
    ...
    return from(dbService.hasXtreamCategories(route.snapshot.params.id, type)).pipe(switchMap((exists) => __async(void 0, null, function* () {
      try {
        if (exists) {
          const localData2 = yield dbService.getXtreamCategories(route.snapshot.params.id, type);
          patchState(store, {
            [stateKey]: localData2,
            isLoadingCategories: false
          });
        ...
        const remoteData = yield dataService.fetchData(`${store.currentPlaylist().serverUrl}/player_api.php`, queryParams);
        if (remoteData && Array.isArray(remoteData)) {
          yield dbService.saveXtreamCategories(route.snapshot.params.id, remoteData, type);
        }
        const localData = yield dbService.getXtreamCategories(route.snapshot.params.id, type);
        patchState(store, {
          [stateKey]: localData,
          isLoadingCategories: false
        });
        ...
        console.error(`Error in fetchCategories for ${type}:`, err);
        patchState(store, { isLoadingCategories: false });
        throw err;
      }
    })));
  };
  const fetchStreams = (action, stateKey, type) => {
    ...
    return from(dbService.hasXtreamContent(route.snapshot.params.id, type)).pipe(switchMap((exists) => __async(void 0, null, function* () {
      try {
        if (exists) {
          const localData2 = yield dbService.getXtreamContent(route.snapshot.params.id, type);
          patchState(store, {
            [stateKey]: localData2,
            isLoadingContent: false
          });
        ...
        if (remoteData && Array.isArray(remoteData)) {
          patchState(store, {
            itemsToImport: store.itemsToImport() + remoteData.length
          });
          const insertedCount = yield dbService.saveXtreamContent(route.snapshot.params.id, remoteData, type, (count) => patchState(store, {
            importCount: count
          }));
          console.log(`Inserted ${insertedCount} items`);
        }
        const localData = yield dbService.getXtreamContent(route.snapshot.params.id, type);
        patchState(store, {
          [stateKey]: localData,
          isLoadingContent: false
        });
  ...
  const searchContent = (_0, _1, ..._2) => __async(void 0, [_0, _1, ..._2], function* (searchTerm, types, route2 = inject(ActivatedRoute), dbService2 = inject(DatabaseService)) {
    if (!route2.snapshot.params.id)
      return;
    const results = yield dbService2.searchXtreamContent(route2.snapshot.params.id, searchTerm, types);
    return results;
  });
  const fetchXtreamPlaylist = rxMethod(pipe(() => {
    const playlistId = route.snapshot.params.id;
    if (!playlistId)
      return EMPTY;
    return from(dbService.getPlaylistById(playlistId)).pipe(switchMap((playlist) => __async(void 0, null, function* () {
      if (playlist) {
        patchState(store, {
          currentPlaylist: playlist
        });
      } else {
        const playlist2 = oldStore.selectSignal(selectActivePlaylist)();
        if (playlist2) {
          yield dbService.createPlaylist(playlist2);
          patchState(store, {
            currentPlaylist: __spreadProps(__spreadValues({}, playlist2), {
              serverUrl: playlist2.serverUrl,
              id: playlist2._id,
              name: playlist2.title
            })
          });
        }
      }
  ...
  const fetchLiveCategories = rxMethod(pipe(switchMap(() => fetchCategories(XtreamCodeActions.GetLiveCategories, "liveCategories", "live"))));
  const fetchVodCategories = rxMethod(pipe(switchMap(() => fetchCategories(XtreamCodeActions.GetVodCategories, "vodCategories", "movies"))));
  const fetchSerialCategories = rxMethod(pipe(switchMap(() => fetchCategories(XtreamCodeActions.GetSeriesCategories, "serialCategories", "series"))));
  const fetchLiveStreams = rxMethod(pipe(switchMap(() => fetchStreams(XtreamCodeActions.GetLiveStreams, "liveStreams", "live"))));
  const fetchVodStreams = rxMethod(pipe(switchMap(() => fetchStreams(XtreamCodeActions.GetVodStreams, "vodStreams", "movie"))));
  const fetchSerialStreams = rxMethod(pipe(switchMap(() => fetchStreams(XtreamCodeActions.GetSeries, "serialStreams", "series"))));
  const setPage = (page) => patchState(store, { page });
  const setLimit = (limit) => patchState(store, { limit });
  ...
  const setSelectedContentType = (type) => patchState(store, {
    selectedContentType: type,
    selectedCategoryId: null
  });
  const setSelectedItem = (item) => patchState(store, { selectedItem: item });
  const searchContentMethod = rxMethod(pipe(switchMap(({ term, types }) => from(searchContent(term, types, route, dbService)).pipe(tap((results) => {
    patchState(store, {
      searchResults: results || []
    });
  })))));
  const fetchVodDetailsWithMetadata = rxMethod(pipe(combineLatestWith(oldStore.select(selectActivePlaylist)), switchMap(([{ vodId, categoryId }, playlist]) => {
    if (!playlist)
      return EMPTY;
    return from(dataService.fetchData(`${playlist.serverUrl}/player_api.php`, {
      ...
      patchState(store, {
        selectedItem: __spreadProps(__spreadValues({}, currentVod), {
          stream_id: vodId
        })
      });
    }));
  })));
  const fetchSerialDetailsWithMetadata = rxMethod(pipe(combineLatestWith(oldStore.select(selectActivePlaylist)), switchMap(([{ serialId, categoryId }, playlist]) => {
    if (!playlist)
      return EMPTY;
    return from(dataService.fetchData(`${playlist.serverUrl}/player_api.php`, {
      ...
      password: playlist.password,
      series_id: serialId
    })).pipe(tap((currentSerial) => {
      patchState(store, {
        selectedCategoryId: Number(categoryId)
      });
      patchState(store, {
        selectedItem: __spreadProps(__spreadValues({}, currentSerial), {
          series_id: serialId
        })
      });
    }));
  })));
  const initializeContent = () => __async(void 0, null, function* () {
    patchState(store, { isImporting: true });
    try {
      console.log("Starting content initialization...");
      console.log("Fetching live categories...");
      yield lastValueFrom(fetchCategories(XtreamCodeActions.GetLiveCategories, "liveCategories", "live"));
      yield lastValueFrom(fetchCategories(XtreamCodeActions.GetVodCategories, "vodCategories", "movies"));
      yield lastValueFrom(fetchCategories(XtreamCodeActions.GetSeriesCategories, "serialCategories", "series"));
      console.log("Fetching content...");
      yield lastValueFrom(fetchStreams(XtreamCodeActions.GetLiveStreams, "liveStreams", "live"));
      yield lastValueFrom(fetchStreams(XtreamCodeActions.GetVodStreams, "vodStreams", "movie"));
      yield lastValueFrom(fetchStreams(XtreamCodeActions.GetSeries, "serialStreams", "series"));
  ...
  const checkPortalStatus = rxMethod(pipe(combineLatestWith(oldStore.select(selectActivePlaylist)), switchMap(([_, playlist]) => {
    if (!playlist)
      return EMPTY;
    return from(dataService.fetchData(`${playlist.serverUrl}/player_api.php`, {
      username: playlist.username,
      ...
      console.log("Portal status response:", response);
  ...
  return {
    ...
    updatePlaylist(playlist) {
      patchState(store, {
        currentPlaylist: __spreadProps(__spreadValues({}, store.currentPlaylist()), {
          playlist
        })
      });
    },
    ...
    loadEpg() {
      return __async(this, null, function* () {
        ...
        const remoteData = yield dataService.fetchData(`${store.currentPlaylist().serverUrl}/player_api.php`, {
        ...
        if (remoteData?.epg_listings && Array.isArray(remoteData.epg_listings)) {
          console.log(`Got epg data categories:`, remoteData);
          patchState(store, {
            epgItems: remoteData.epg_listings.map((i) => __spreadProps(__spreadValues({}, i), {
              title: b64DecodeUnicode(i.title).trim(),
              description: b64DecodeUnicode(i.description).trim()
            }))
          });
        } else {
          console.error("Invalid remote data received:", remoteData);
          patchState(store, {
            epgItems: []
          });
          return [];
    ...
    loadChannelEpg(streamId) {
      return __async(this, null, function* () {
        try {
          const response = yield dataService.fetchData(`${store.currentPlaylist().serverUrl}/player_api.php`, {
            action: "get_short_epg",
            username: store.currentPlaylist().username,
            password: store.currentPlaylist().password,
            stream_id: streamId,
          ...
          if (response?.epg_listings && Array.isArray(response.epg_listings)) {
            return response.epg_listings.map((item) => __spreadProps(__spreadValues({}, item), {
              title: b64DecodeUnicode(item.title).trim(),
              description: b64DecodeUnicode(item.description).trim()
            }));
          }
          return [];
        } catch (error) {
    ...
    setHideExternalInfoDialog(hideExternalInfoDialog) {
      localStorage.setItem("hideExternalInfoDialog", store.hideExternalInfoDialog.toString());
...
export {
  ...
  require_moment,
  playlistReducer,
  selectIsEpgAvailable2 as selectIsEpgAvailable,
  selectActive2 as selectActive,
  selectCurrentEpgProgram2 as selectCurrentEpgProgram,
  selectChannels2 as selectChannels,
  selectPlaylistsLoadingFlag2 as selectPlaylistsLoadingFlag,
